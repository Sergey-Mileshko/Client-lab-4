...
function loadSources(){
    const url = 'https://newsapi.org/v2/sources?apiKey=c7b2176696be49a18d4ad0a9494318d8';
...
function createNewsItem(token, data){
    token.querySelector('.news__picture').style.backgroundImage = `url(${data.urlToImage ? data.urlToImage : 'Src/nnn/n.png'})`;
...
function loadBy(urlPart){
    hide('#error-block');
    const url = 'https://newsapi.org/v2/' + urlPart + 'apiKey=c7b2176696be49a18d4ad0a9494318d8';
    const request = new Request(url);
    fetch(request)
        .then(function(response) { return response.json(); })
        .then(function(data) {
